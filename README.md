...
- **Outline & thumbnails** — `Ctrl+T` toggles a sidebar between table of contents and page thumbnails; drag thumbnails to reorder pages, drop a PDF between them to insert its pages, drag a chapter in the outline to move all of its pages at once, or drag pages **out to a file manager** to export them as a standalone PDF (annotations and notes included), like macOS Preview
