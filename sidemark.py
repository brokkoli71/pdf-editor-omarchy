...
def nearest_vertex_point(shapes, x, y, hit, held=()):
...
def nearest_edge_point(shapes, x, y, hit, held=()):
    """Nearest point ON an edge of `shapes`, within `hit` — the projection of
    (x, y) onto the closest segment.

    Segments touching a held vertex are skipped: the point in your hand always
    lies on its own two edges, so without this it would snap to itself and
    never move. This is a POSITIONAL snap only — the point lands there and is
    then an ordinary vertex. Nothing binds it to the edge, and if that edge
    later moves the point stays put; a real binding needs stored constraints
    and stable stroke ids (row 129)."""
    best, at = hit, None
    for st, verts in shapes:
        closed = len(verts) > 2
        n = len(verts)
        for i in range(n if closed else n - 1):
            j = (i + 1) % n
            if (id(st), i) in held or (id(st), j) in held:
                continue
            ax, ay = verts[i]
            bx, by = verts[j]
            vx, vy = bx - ax, by - ay
            ...
            if span < 1e-12:
                continue
            t = max(0.0, min(1.0, ((x - ax) * vx + (y - ay) * vy) / span))
...
class _SyntheticDrag:
    ...
    def __init__(self, x, y, button, shift=False):
        self._start = (x, y)
        self._button = button
        self._state = (Gdk.ModifierType.SHIFT_MASK if shift
...
class PDFCanvas(Gtk.DrawingArea):
    ...
    def __init__(self, interactive=True):
        ...
        # snap targets while the pen still holds a freshly recognised shape's
        ...
        if self._zoom_selecting:
            ...
            if self._straight_mode:
                if self._snap_kind in ("line", "path", "polygon"):
                    ...
                    self._live_snap_at = self._live_snap_target(
                        sx + offset_x, sy + offset_y)
                    if self._live_snap_at:
                        pt = self._screen_to_pdf(*self._live_snap_at)
    ...
    def _snap_reach(self):
        return vertex_snap_radius(self.get_width(), self.get_height())

    def _vertex_snap_target(self, sx, sy):
        """Where the dragged control point should sit — another control point
        if one is in reach, else a point on an edge. Only the selection's own
    ...
    def _page_shapes_screen(self):
        ...
        the snap targets offered while DRAWING, where there is no selection to
        take them from."""
        out = []
    ...
    def _held_vertex_index(self):
        ...
        of an open shape, and index 0 of a closed ring — the shared first/last
        point, so the ring cannot tear."""
        return (0 if self._snap_kind == "polygon"
                else len(self.current_stroke) - 1)

    def _live_snap_all(self):
        """Every snap target for the held control point: the page's shapes AND
        the live shape's OWN other vertices.

        Its own matter as much as the page's — that is how you close a path
        onto its start, or bring an end back to an earlier corner, without
        lifting. The held point is excluded by _live_snap_target, and
        nearest_edge_point drops the segments touching it, so it can never snap
        to itself or to the edge it is attached to."""
        verts = shape_vertices(self.current_stroke)
        if not verts:
            return self._live_snap_shapes
        live = (self, [self._pdf_to_screen(x, y) for x, y in verts])
        return self._live_snap_shapes + [live]

    def _live_snap_target(self, sx, sy):
        """The snap for the control point the pen still holds after a dwell.

        Page targets come from the whole PAGE, not a selection: the shape being
        drawn is not committed yet, so there is nothing selected to aim at.
        This is what lets a freshly recognised path join up with what is
        already on the page — or with itself — without lifting the pen."""
    ...
    def _draw_lasso(self, ctx):
        ...
        # noise, and what you need to see is what you can actually hit.
        if self._straight_mode and self._snap_kind in ("line", "path",
                                                        "polygon"):
            near = self._snap_reach() * 3.0
            mx, my = self._mouse_x, self._mouse_y
            held = (id(self), self._held_vertex_index())
            shown = [(vx, vy) for st, verts in self._live_snap_all()
                     for i, (vx, vy) in enumerate(verts)
                     if (id(st), i) != held
                     and math.hypot(vx - mx, vy - my) <= near]
            draw_shape_vertices(ctx, shown, self.zoom_accent)
            if self._live_snap_at:
                draw_vertex_snap_ring(ctx, self._live_snap_at, self.zoom_accent)
...
class TextPageView(Gtk.Overlay):
    ...
    def __init__(self, font_px=13):
        super().__init__()
        # no active tool: `bindings` says what each button does (row 132) and
        # `tool` below means "the press in flight", falling back to the left
        ...
        # No gesture of its own: the press router below drives _on_ink_*, so
        ...
        # chord through the binding table and either claims the press for that
        # tool, or DENIES so the caret keeps it (text selection, the context
        # menu, clicking to type). One router is what keeps a chord from
        # meaning one thing here and another on a PDF page.
        router = Gtk.GestureDrag()
        router.set_button(0)
        router.set_propagation_phase(Gtk.PropagationPhase.CAPTURE)
        router.connect("drag-begin", self._on_press_begin)
        router.connect("drag-update", self._on_press_update)
        router.connect("drag-end", self._on_press_end)
    ...
    def set_tool(self, tool):
        """Put `tool` on the left button and follow it with the cursors.
    ...
    def _begin_pan(self):
        """Grab-pan the sheet: the content follows the pointer, so the scroll
        offset moves opposite the drag delta (see _pan_update)."""
        self._panning = True
        ha, va = self.scroll.get_hadjustment(), self.scroll.get_vadjustment()
    ...
    def _on_thumb_event(self, ctrl, event):
        ...
        if t == Gdk.EventType.BUTTON_PRESS and event.get_button() == BTN_THUMB:
            ...
            self._thumb_gesture = _SyntheticDrag(
                *self._mouse_xy, BTN_THUMB, shift=shift)
    ...
    def _on_sheet_scroll(self, ctrl, dx, dy):
        ...
        if ctrl_held or self._thumb_gesture is not None:
            ...
            if self._panning:
                # rebase the pan so the next motion event doesn't jump
                self._pan_start = new_scroll
                self._thumb_gesture._start = self._mouse_xy
            return True
    ...
    # ── the press router (row 132) ───────────────────────────────────────────

    def _on_press_begin(self, gesture, x, y):
        """Resolve this press through the binding table and give it to that
        tool — or DENY, and the caret below keeps it.
        ...
        tool = self.bindings.tool_for(btn, ctrl, shift, alt, mode="text")
        plain = self.bindings.tool_for(btn, mode="text")
        additive = False
        # THE exception (PDF-canvas parity): with the lasso on the plain button
        # Shift ADDS to the selection rather than running what Shift is bound
        # to — Shift+lasso is still the lasso.
        if shift and not ctrl and not alt and plain == "lasso":
            tool, additive = "lasso", True

        # A live selection is grabbable with ANY tool, and it MUST be claimed
        # here: a pasted image would otherwise be unmovable until you fetched
        # the lasso (row 125).
        grab = self.selection_grab_at(x, y)
        ...
        if btn == BTN_RIGHT and tool == "eraser":
            # A right press is ambiguous until it moves: erase on a drag, the
            # context menu on a clean click. Claim it up front anyway — the
            # TextView opens its menu on the PRESS, so a deferred claim would
            # race the popup — and re-open the menu ourselves on release.
            gesture.set_state(Gtk.EventSequenceState.CLAIMED)
        ...
        if tool == "pan":
            self._begin_pan()
            return
        self._ink_additive = additive
        self._on_ink_begin(gesture, x, y)

    def _on_press_update(self, gesture, dx, dy):
        ...
        if self._vertex_drag:
            ...
            if self._straight_mode:
                if self._snap_kind in ("line", "path", "polygon"):
                    ...
                    self._live_snap_at = self._live_snap_target(x, y)
                    tx, ty = self._live_snap_at or (x, y)
                    idx = (0 if self._snap_kind == "polygon"
                           else len(self.current_stroke) - 1)
                    self.current_stroke = move_shape_vertex(
    ...
    def _snap_reach(self):
        return vertex_snap_radius(self.ink.get_width(), self.ink.get_height())

    def _vertex_snap_target(self, x, y):
        """PDFCanvas._vertex_snap_target's twin — a control point if one is in
    ...
    def _live_snap_all(self):
        """The page's shapes plus the live shape's own vertices — see
        PDFCanvas._live_snap_all."""
        verts = shape_vertices(self.current_stroke)
        if not verts:
            return self._live_snap_shapes
        return self._live_snap_shapes + [(self, verts)]

    def _live_snap_target(self, x, y):
        """Snap for the control point the pen still holds after a dwell — see
        PDFCanvas._live_snap_target for why the page is the target set, and
        _live_snap_all for why the live shape is in it too."""
        if not self._live_snap_shapes and not self.current_stroke:
            return None
    ...
    def _draw_lasso(self, ctx, drag, wf):
        ...
        if self._straight_mode and self._snap_kind in ("line", "path",
            ...
            near = self._snap_reach() * 3.0
            mx, my = self.current_stroke[-1] if self.current_stroke else (0, 0)
            held = (id(self), self._held_vertex_index())
            shown = [(vx, vy) for st, verts in self._live_snap_all()
                     for i, (vx, vy) in enumerate(verts)
                     if (id(st), i) != held
                     and math.hypot(vx - mx, vy - my) <= near]
            draw_shape_vertices(ctx, shown, self.accent())
            if self._live_snap_at:
                draw_vertex_snap_ring(ctx, self._live_snap_at, self.accent())
        if self._lassoing and len(self._lasso_path) >= 2:
            ctx.set_source_rgba(ar, ag, ab, 0.9)
            ctx.set_line_width(1.5)
            ctx.set_dash([5.0, 3.0])
...
class PDFEditorWindow(Adw.ApplicationWindow):
    ...
    def _enter_text_mode(self, s=None):
        ...
        if s is self._active_session:
            ...
            # table is shared with PDF pages (row 132), and silently rewriting
            # it whenever a text page opens would undo the user's own binding.
            # Typing works regardless — keys go to the focused view; it is
